...
**2. Table Creation and loading data**
```sql
create table stores(
...
**3. Improving Query Performance- useful for large data**
```sql
...
**10. For each store, identify the best-selling day based on highest quantity sold.- Using Sub-query**
```sql
select *
from (select 
...
**11. Identify the least selling product in each country for each year based on total units sold.- Using CTE**
```sql
